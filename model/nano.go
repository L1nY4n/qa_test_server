...
//设备的种子开关状态
type Nano_Dev_Laser_status struct {
...
/*温度监控*/
type Nano_Dev_temperature_mon struct {
...
/*告警监控模块*/
type Nano_Dev_alarm_mon struct {
...
type Nano_laser_mon struct {
	HeadA     uint8
	HeadB     uint8
	Mon_laser Nano_Dev_Laser_status    `json:"激光器状态监测" gorm:"embedded"`
	Mon_cur   Nano_Dev_cur_mon         `json:"激光器电流监测" gorm:"embedded"`
	Mon_vol   Nano_Dev_vol_mon         `json:"激光器电压监测" gorm:"embedded"`
	Mon_temp  Nano_Dev_temperature_mon `json:"激光器温度监测" gorm:"embedded"`
	Mon_tcm   [3]Nano_Dev_tcm_mon      `json:"激光器温控板监测" gorm:"embedded"`
	Mon_seed  Nano_Dev_seed_mon        `json:"激光器种子模块监测" gorm:"embedded"`
	Mon_alarm Nano_Dev_alarm_mon       `json:"激光器告警监测" gorm:"embedded"`
	Mon_fpga  Nano_Dev_fpga_mon        `json:"激光器FPGA寄存器监测" gorm:"embedded"`
	Mon_time  Dev_time                 `json:"激光器时间监测" gorm:"embedded"`
}

//------------------------------------存储结构体-------------------------------------//
type Nano_Product_info struct {
...
type Nano_Sys_conf_check struct {
...
type Nano_Tcm_control_param struct {
...
type Nano_Mon_limit_param struct {
	//阈值
...
type Nano_Sys_init_param_struct struct {
	HeadA      uint8               `json:"包头1"`
	HeadB      uint8               `json:"包头2"`
	Pro_info   Nano_Product_info   `json:"产品信息"  gorm:"embedded"`
	Seed_param Nano_Seed_param     `json:"种子信息"  gorm:"embedded"`
	Sys_cc     Nano_Sys_conf_check `json:"设备标志位"  gorm:"embedded"`
	//public fpga_init_param fpga_init_p;
	Pd_param  Nano_PD_param          `json:"pd参数"  gorm:"embedded"`
	Temp      Nano_Temp_param        `json:"温度参数"  gorm:"embedded"`
	Tcm       Nano_Tcm_control_param `json:"温控参数"  gorm:"embedded"`
	Cur       Nano_Cur_param         `json:"电流参数"  gorm:"embedded"`
	Mon_limit Nano_Mon_limit_param   `json:"监控限定值"  gorm:"embedded"`
